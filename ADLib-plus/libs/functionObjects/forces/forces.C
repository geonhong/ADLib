...
{
    defineTypeNameAndDebug(nforces, 0);
    addToRunTimeSelectionTable(functionObject, nforces, dictionary);
}
}
...
Foam::word Foam::functionObjects::nforces::fieldName(const word& name) const
{
    return this->name() + ":" + name;
}


void Foam::functionObjects::nforces::createFiles()
{
...
void Foam::functionObjects::nforces::writeIntegratedHeader
(
    const word& header,
    Ostream& os
) const
...
void Foam::functionObjects::nforces::writeBinHeader
(
    const word& header,
    Ostream& os
) const
...
void Foam::functionObjects::nforces::setCoordinateSystem
(
...
void Foam::functionObjects::nforces::initialise()
{
    if (initialised_)
    {
        return;
...
void Foam::functionObjects::nforces::initialiseBins()
{
    if (nBin_ > 1)
    {
...
void Foam::functionObjects::nforces::resetFields()
{
    force_[0] = Zero;
    force_[1] = Zero;
    force_[2] = Zero;
...
Foam::functionObjects::nforces::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::nforces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::nforces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::nforces::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::nforces::applyBins
(
...
void Foam::functionObjects::nforces::addToFields
(
    const label patchi,
    const vectorField& Md,
    const vectorField& fN,
...
void Foam::functionObjects::nforces::writeIntegratedForceMoment
(
...
void Foam::functionObjects::nforces::writeForces()
{
...
void Foam::functionObjects::nforces::writeBinnedForceMoment
(
...
void Foam::functionObjects::nforces::writeBins()
{
...
Foam::functionObjects::nforces::nforces
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
bool Foam::functionObjects::nforces::read(const dictionary& dict)
{
	...
	if (dict.found("set"))
	{
		...
		word setName = dict.get<word>("set");
		...
		if (objs.found(setName))
		{
			...
			Info<< "faceSet size = " << faceSet_.size() << endl;
		}
	}
	else
	{
...
void Foam::functionObjects::nforces::calcForcesMoment()
{
    initialise();

    resetFields();

	Info<< "directForceDensity " << directForceDensity_ << endl;
    ...
    if (directForceDensity_)
    {
		...
		Info<< "isSet " << isSet_ << " " << patchSet_.size() << endl;

		if (isSet_)
		{
			vectorField Md(patchSet_.size(), Zero);
			vectorField fN(patchSet_.size(), Zero);
			vectorField fT(patchSet_.size(), Zero);
			vectorField fP(patchSet_.size(), Zero);

			forAll(patchSet_, ind)
			{
				label facei = patchSet_[ind];
				label pi = 0;
				label fi = 0;

				forAll(boundary, bi)
				{
					const fvPatch& patch = boundary[bi];

					label start = patch.start();
					label size = patch.size();

					if (facei >= start && facei < start+size)
					{
						pi = bi;
						fi = facei-start;

						Info<< "Global face index " << facei << " corresponds to "
							<< "patch " << patch.name() << "(" << pi << ")" 
							<< " and face " << fi
							<< endl;
						break;
				...
				vector Mdi
				(
				 	mesh_.C().boundaryField()[pi][fi] - coordSys_.origin()
				);

				scalar sA(mag(Sfb[pi][fi]));

				vector fNi
				(
				 	Sfb[pi][fi]/sA
				   *(
					   Sfb[pi][fi] & fD.boundaryField()[pi][fi]
					)
				);

				vector fTi(sA*fD.boundaryField()[pi][fi] -fNi);
			...
			vectorField Cb(patchSet_.size(), Zero);

			applyBins(Md, fN, fT, fP, Cb);
		}
		else
		{
			for (const label patchi : patchSet_)
    ...
    else
    {
		...
		if (isSet_)
		{
			vectorField Md(faceSet_.size(), Zero);
			vectorField fN(faceSet_.size(), Zero);
			vectorField fT(faceSet_.size(), Zero);
			vectorField fP(faceSet_.size(), Zero);

			label ind = 0;

			for (const label facei : faceSet_)
			{
				label pi = 0;
				label fi = 0;
				...
				vector fNi
				(
				 	rho(p)*Sfb[pi][fi]*(p.boundaryField()[pi][fi] - pRef)
				);

				vector fTi(Sfb[pi][fi] & devRhoReffb[pi][fi]);
			...
			vectorField Cb(faceSet_.size(), Zero);
...
Foam::vector Foam::functionObjects::nforces::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


Foam::vector Foam::functionObjects::nforces::momentEff() const
{
    return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
}


bool Foam::functionObjects::nforces::execute()
{
    calcForcesMoment();
...
bool Foam::functionObjects::nforces::write()
{
    if (writeFields_)
    {
